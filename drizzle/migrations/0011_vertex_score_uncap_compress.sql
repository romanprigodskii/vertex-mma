-- Wave 3.5 step 3.7: uncap component sub-stats + soft-compress finals.
--
-- Previous behaviour: every Striking / Grappling sub-stat was capped at its
-- nominal max (35, 25, 25, 15 etc.), and the totals were capped at 100. That
-- prevented elite specialists from compounding their advantages — Pereira's
-- KO finish rate and SLpM both maxed, but no further reward for being well
-- above the cap; same with Khabib's TD avg / Sub avg.
--
-- This migration:
--   1. Removes the per-sub-stat LEAST() ceilings inside Striking + Grappling.
--   2. Removes the LEAST(100, ...) total ceilings on Win Quality, Striking,
--      and Grappling. (Win Quality keeps its inner LEAST(wins, 25) sanity
--      cap so a 30-0 fighter doesn't double-count their last 5 wins.)
--   3. Computes raw_current_total / raw_all_time_total uncapped, then
--      applies soft compression above 90:
--          final = raw                          if raw <= 90
--          final = 90 + (raw - 90) * 0.6        otherwise
--      then LEAST(100, ...) as a theoretical hard ceiling.
--
-- 100 is only reachable when raw > 106.66 — i.e. a fighter who maxes every
-- weighted component AND eats the champion bonus AND has zero penalty. No
-- real fighter hits this; the value remains symbolic.
...
components AS (
  SELECT
    ...
    -- Win Quality: keep LEAST(wins, 25) sanity cap; drop outer LEAST(100,...).
    GREATEST(0,
      (
    ...
    -- Striking — no per-sub-stat or total caps. Pereira / Aspinall etc.
    -- can push past 100 on this component alone; that's by design.
    (GREATEST(0,
      COALESCE(fsa.slpm, 0)::float / 5.5 * 35.0
      + COALESCE(fsa.str_acc, 0)::float / 0.55 * 25.0
      + COALESCE(fsa.str_def, 0)::float / 0.65 * 25.0
      + COALESCE(
    ...
    (GREATEST(0,
      COALESCE(fsa.td_avg, 0)::float / 4.0 * 25.0
      + COALESCE(fsa.td_def, 0)::float / 0.80 * 30.0
      + COALESCE(fsa.sub_avg, 0)::float / 2.5 * 25.0
      + COALESCE(
...
SELECT
  ...
  CASE
    WHEN is_active AND ufc_bouts >= 3 THEN
      LEAST(100, GREATEST(0, ROUND(
        CASE
          WHEN raw_current_total <= 90 THEN raw_current_total
          ELSE 90.0 + (raw_current_total - 90.0) * 0.6
        END
      )))::integer
    ELSE NULL
  END AS vertex_score,
  ...
  CASE
    WHEN ufc_bouts >= 3 THEN
      LEAST(100, GREATEST(0, ROUND(
        CASE
          WHEN raw_all_time_total <= 90 THEN raw_all_time_total
          ELSE 90.0 + (raw_all_time_total - 90.0) * 0.6
        END
      )))::integer
    ELSE NULL
