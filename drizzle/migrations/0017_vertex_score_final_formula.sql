-- Wave 3.5 step 5B: final Vertex Score formula.
--
-- Adds three new score components on top of step 5A's opponent-quality
-- foundation:
--
--   Era Dominance         — fighter.era_dominance (0-100) populated by
--                           scripts/compute_era_dominance.ts. Separates
--                           "12 title fights" (Jones, Anderson, DJ, GSP)
--                           from "3 title fights" (short-tenure current
--                           champs) once Quality Wins has capped at 100.
--   Performance Diff.     — SLpM-SAPM and TD differential, derived from
--                           fighter_stats_aggregate. Materialized back to
--                           fighter.performance_differential.
--   Finishing Dominance   — KD per fight, sub attempts per fight, KO win
--                           rate, sub win rate. Derived from bout +
--                           bout_round_stats. Materialized back to
--                           fighter.finishing_dominance.
--
-- Final weights (each row's positives sum to 1.00 before penalty):
--
--   CURRENT
--     QualityWins              0.30
--     ChampionshipPedigree     0.18
--     EraDominance             0.12
--     PerformanceDifferential  0.10
--     FinishingDominance       0.15
--     Activity                 0.15
--     RecentLossPenalty       -0.10
--
--   ALL-TIME
--     QualityWins              0.28
--     ChampionshipPedigree     0.22
--     EraDominance             0.22
--     PerformanceDifferential  0.12
--     FinishingDominance       0.16
--     TotalLossPenalty        -0.10
--
-- Sum of positive weights = 1.00 in both. Final score is clamped to
-- [0, 100] via GREATEST/LEAST.
...
-- Performance Differential: SLpM - SAPM scaled, blended 55/45 with the
-- takedown differential. +2.5 sig-str diff per minute ≈ 100; 0 ≈ 50;
-- -2.5 ≈ 0. Same scale on TD diff. Both signal "do you outperform your
-- opponents per round of fight time."
performance_diff AS (
  SELECT
    fsa.fighter_id,
...
-- Finishing Dominance: KD per fight + sub attempts per fight + KO/sub
-- win rate among UFC wins. Each component capped at its weight ceiling.
finishing_dom AS (
  SELECT
    f.id AS fighter_id,
...
components AS (
  SELECT
    ...
    COALESCE(f.era_dominance, 0)::float AS era_dominance,
