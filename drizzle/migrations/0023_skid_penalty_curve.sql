-- Wave 3.5 step 6A.4: rebalance current multiplier curve + add skid penalty.
-- All-time formula and tier breaks (80/60/40) untouched.
--
-- Step 6A.3 issues being fixed:
--   1. Cejudo at 67 with a 4-loss skid sat in Elite. Real-world result
--      should be Veteran/Roster regardless of his championship pedigree.
--   2. Middle contenders (Merab/Yan/Pantoja 86-87) overtook active
--      champion Pereira (85). The 6A.3 multiplier ×1.4 for raw<60 over-
--      lifted contenders while the 60-80 bucket compressed champions —
--      raw_64 (contender) and raw_72 (champion) both ended in the 86-87
--      band.
--
-- New multiplier curve (preserves order, no compression in 65-85 band):
--    raw <40   → ×1.3            (gentle bottom lift)
--    raw <65   → 52 + (raw-40)*1.25   (mid lift; from 52 → 83)
--    raw <85   → 83 + (raw-65)*1.0    (LINEAR — preserves champion spread)
--    raw ≥85   → 103 + (raw-85)*0.4   (soft cap toward 100)
--   Plus LEAST(100, ...) outer clamp so no value exceeds the integer ceiling.
--
-- Skid penalty (post-multiplier):
--   losses_last_3 >= 3 → final - 25, then GREATEST(0, ...).
-- Applied AFTER the multiplier so it has real bite at every score level.
...
multiplied AS (
  SELECT
    *,
    -- Step 6A.4 multiplier curve. Bucket boundaries chosen so the
    -- transitions are continuous (52 at raw=40, 83 at raw=65, 103 at
    -- raw=85). LEAST(100, ...) clamps the top so the soft-cap region
    -- effectively saturates around raw=85 for the small handful of
    -- fighters who get there.
    CASE
...
SELECT
  ...
  -- Skid penalty: 3+ losses in the most recent 3 UFC fights → -25 final.
  -- Applied post-multiplier so a 4-loss-skid champion (Cejudo) drops
  -- from Elite into Veteran/Roster regardless of his pedigree.
  CASE
