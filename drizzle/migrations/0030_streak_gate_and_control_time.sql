-- Wave 6C.3: fighter_vertex_score view rewrite — two orthogonal changes.
--
-- (a) Streak gate. The streak_bonus from Wave 6C.2 fired any time a
--     fighter had current_streak >= 3, which let unranked finishers
--     (Walker, Salkilld, Ziam, Nolan, Garcia, Mason Jones, Gautier,
--     Pyfer, Soriano, Malkoun) ride the bonus into the top of HW/LW/MW
--     despite quality_wins ≈ 0. Adding `AND quality_wins >= 20` to the
--     gate keeps the bonus for fighters with at least ~1 strong/top5
--     win on record while dropping pure-rookie streakers out of view.
--
-- (b) Control time in performance_diff. The component used to be a 55/45
--     mix of striking-differential and takedown-differential. UFCStats
--     `bout_round_stats.control_time_seconds` was already in the DB but
--     unused by the formula — a takedown without control is largely
--     cosmetic; a takedown with 3 minutes of control is dominance.
--     New split: 0.45 striking + 0.35 control + 0.20 takedown.
--
-- Both edits live inside the same CREATE VIEW so the existing
-- DROP/CREATE pattern from 0025/0029 stays linear.
--
-- Locked (unchanged): tier breaks, all-time formula weights, soft-mult
-- curve, skid penalty, ufc_bouts gate, activity buckets, recent_loss
-- math, champion-tier multipliers, finishing_dominance.
...
-- Wave 6C.3: per-fighter career totals from bout_round_stats. Each round
-- attended contributes either 300 seconds (round went the distance) or
-- the bout's time_finished_seconds (final round of a finish). Both are
-- already populated by the UFCStats scraper for completed bouts.
...
performance_diff AS (
  SELECT
    ...
    LEAST(100, GREATEST(0, ROUND(
        ...
        -- 30 sec/min baseline → 50; 90 sec/min (Khabib-tier) → 110 (clamped
        -- to 100 by inner LEAST); 0 sec/min → 20. NULL/no rounds → 50
        -- so non-scraped fighters don't get penalised.
      + 0.35 * (
          CASE
            WHEN ctp.total_fight_seconds IS NULL OR ctp.total_fight_seconds = 0 THEN 50
            ELSE LEAST(100, GREATEST(0,
              50 + ((ctp.total_control_seconds / ctp.total_fight_seconds * 60.0) - 30.0) * 1.0
            ))
          END
        )
...
raw_scores AS (
  SELECT
    *,
    -- Wave 6C.3: streak bonus is now gated on quality_wins >= 20 so it
    -- doesn't pull pure-rookie streakers into the top of a division.
    -- ~20 = 1 strong/top5 win, 2 solid wins, or ~7 ranked wins.
    CASE
