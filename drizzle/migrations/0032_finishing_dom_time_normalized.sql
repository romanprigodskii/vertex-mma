-- Wave 6E: time-normalize finishing_dominance.
--
-- The 0031 `finishing_dom` CTE computed:
--   kd_per_fight = SUM(brs.knockdowns)   / COUNT(DISTINCT b.id)
--   sa_per_fight = SUM(brs.sub_attempts) / COUNT(DISTINCT b.id)
-- which over-credited fast finishers: 1 KD in a 30-second R1 stoppage counts
-- the same as 1 KD across a 15-minute decision, even though the first is
-- ~30× denser per actual time-on-the-mat.
--
-- 0032 switches the per-bout denominator to total fight seconds (using the
-- same `bout.round_finished` + `bout.time_finished_seconds` formulation
-- already used by `control_time_per_fighter` in 0031, so the time math is
-- consistent across CTEs), and converts to per-15-minute rates (900 s):
--   kd_per_15min = total_kd / total_seconds * 900
--   sa_per_15min = total_sa / total_seconds * 900
--
-- Calibration: top KO/sub finishers (Aspinall, Khabib, Conor 2015, Adesanya,
-- Volkanovski) target finishing_dominance_score ≈ 80-95. New multipliers in
-- `components.finishing_dominance_score` were tuned against the
-- post-materialization spot-check diagnostic.
--
-- `ko_rate` and `sub_rate` are method-based ratios — left unchanged at ×35.
--
...
components AS (
  SELECT
    ...
    -- Wave 6E: rate part uses per-15-minute KD / SA, weighted to land top
    -- finishers (Aspinall, Khabib sub-game, Adesanya, Conor 2015) at
    -- ~80-95 when combined with the unchanged method-rate part below.
    LEAST(100, GREATEST(0,
      CASE
        WHEN COALESCE(fd.total_seconds, 0) > 0 THEN
          (fd.total_kd / fd.total_seconds * 900.0) * 14.0
        + (fd.total_sa / fd.total_seconds * 900.0) * 14.0
        ELSE 0
      END
