-- Wave 6E.1: realized-finish rates in finishing_dominance_score.
--
-- 6E (migration 0032) introduced per-15-minute KD / SA rates, which still
-- under-credited high-closure-rate submission specialists. UFC's
-- `sub_attempts` counter measures *attempts*, not closures, so Khabib's
-- pattern (high sub-win count, mediocre sub-attempt rate) scored only 32
-- on finishing_dominance — well below the ~70-85 his record actually
-- supports.
--
-- 0033 adds two new terms to the `finishing_dom` CTE:
--   ko_wins_per_15min  = (KO/TKO wins) / total_seconds * 900
--   sub_wins_per_15min = (sub wins)    / total_seconds * 900
-- and rebalances the multipliers across the now-six terms:
--   kd_per_15min       ×10   (was ×14)
--   sa_per_15min       ×10   (was ×14)
--   ko_rate            ×25   (was ×35)
--   sub_rate           ×25   (was ×35)
--   ko_wins_per_15min  ×20   (new)
--   sub_wins_per_15min ×20   (new)
-- Sum-of-multipliers budget ≈ unchanged (was 98, now 110, bounded by
-- LEAST(100) cap; top finishers stay in the 80-95 band per the 6E spec).
--
-- The realized-finish rate complements the existing signals:
--   - kd_per_15min     : how often you stagger people (attempt-based)
--   - sa_per_15min     : how often you go for subs (attempt-based)
--   - ko_rate          : fraction of wins that are KOs (binary, per-win)
--   - sub_rate         : fraction of wins that are subs (binary, per-win)
--   - ko_wins_per_15min: how dense your KO wins are in fight-time
--   - sub_wins_per_15min: how dense your sub wins are in fight-time
-- The last two close the loop on Khabib-style "tried-rarely-closed-often"
-- patterns the attempt-based rates can't see.
--
...
-- recent_loss math, ufc_bouts gate, control-part calibration all untouched.
