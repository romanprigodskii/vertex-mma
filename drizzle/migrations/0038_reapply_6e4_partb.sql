-- Wave 6E.4 Part B RE-APPLIED (per dev request, post-revert).
--
-- Migration 0036 reverted Part B because the bundle of five coupled
-- changes deformed the rankings (WW top-1 changed, 27 top-5 movers >15).
-- Dev has decided to re-apply the original spec verbatim to inspect the
-- broken state and diagnose specific fighters. NO calibration changes.
--
-- This migration body matches the implementation that produced those
-- numbers (the uncommitted 0035 from the prior session) so the state
-- under inspection is identical to what triggered the STOP condition:
--
--   1. performance_diff_current — last-5-bouts opp-adjusted dominance
--   2. finishing_dominance_current — opp-weighted realized-finish rate
--      (with kd/sa fallback for NULL method, matching the existing
--       all-time finishing_dom CTE's data-quality fallback)
--   3. era_dominance_current — title_fight_count * 10 dropped, replaced
--      with binary +5 "title fight in last 24mo" flag on top of the
--      back-out of active+double bonus from f.era_dominance. The
--      back-out is wrapped in GREATEST(0, ...) separately from the
--      +5 bonus so capped-era fighters (TF≥10: Volk, Jones) still
--      receive the recent-title bonus.
--   4. activity — smooth linear fade replacing bucket-based step jumps
--   5. activity weight reduction 0.18 → 0.12 (raw budget tightens 0.06)
--   6. streak_bonus quality-scaled by streak_opp_avg_tier / 15 (cap 1.0)
--
-- Locked-in (unchanged): all-time formula, tier breaks, multiplier
-- curve, skid penalty, ufc_bouts gate, recent_loss_penalty math,
-- championship_pedigree, quality_wins_score multipliers.
...
components AS (
  SELECT
    ...
    -- Wave 6E.4: opp-weighted recent finish rate (per 15 min × 30).
    CASE
    ...
    END::float AS finishing_dominance_current
  FROM fighter f
    ...
    CASE
      WHEN is_active AND ufc_bouts >= 5 THEN
        GREATEST(0,
          ...
          + finishing_dominance_current * 0.12
          + (CASE
              WHEN current_streak >= 3 AND quality_wins >= 20
              THEN LEAST(current_streak, 5)::float * 4.0
                   * LEAST(1.0, GREATEST(0.0, streak_opp_avg_tier / 15.0))
