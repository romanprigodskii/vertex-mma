-- Wave 6E.4.3: finishing_dominance hybrid (recent + career) + weight cut.
--
-- Wave 6E.4 Part B's pure-recent finishing_dominance_current returned 0
-- for 30+ top-ranked fighters with legitimate career finishing track
-- records (Yan, Shevchenko, Volkanovski, Usman, Sterling, Belal,
-- Namajunas, Moreno, Figueiredo, Gastelum, Blaydes, Dariush, etc.).
-- Root cause: 5 consecutive decision wins — common at championship
-- level — zeros out the recent realized-finish signal entirely, even
-- when the fighter has a strong career KO/sub record.
--
-- This migration introduces a 50/50 hybrid:
--   - 50% recent opp-weighted finish rate (unchanged from 6E.4 Part B)
--   - 50% career signal: knockdowns + sub_attempts per UFC bout × 35
--     each, plus KO/sub win rate × 90 each, summed and clamped to 100.
--     (Initial spec's ×14 / ×35 multipliers produced career signals
--      ≈12-22 for elite-but-recently-decisioning fighters Yan/Volk/Shev;
--      bumped ×2.5 to reach the dev's targeted 30-70 finc range.)
--
-- The career signal is sourced from the existing finishing_dom CTE
-- (which already sums brs.knockdowns/sub_attempts and derives ko_rate/
-- sub_rate) joined to ufc_record for the per-fight denominator. The
-- fighter_stats_aggregate wins_ko/wins_sub method counters are empty
-- in the current data, so they're not used here.
--
-- Also reduces finishing_dom weight in raw_current from 0.12 to 0.10.
-- Freed 0.02 budget is unassigned — raw_current max tightens slightly.
--
-- Locked-in (unchanged): all-time formula (still uses career-only
-- finishing_dominance_score), other component weights, multiplier
-- curve, skid penalty, ufc_bouts gates, recent_loss_penalty math,
-- championship_pedigree (with 6E.2 sticky rule), quality_wins_score
-- multipliers, streak quality scaling (from 6E.4 Part B), era_dominance
-- current/all-time, performance_diff_current, activity smooth fade.
...
-- Wave 6E.4: average opp_tier across the active winning streak. Reads
-- the signed current_streak from fighter (positive = win streak length,
-- negative = losing streak length). The CTE only fires for fighters
-- with current_streak ≥ 1; the bonus itself gates at ≥ 3 downstream.
streak_opp_quality AS (
  SELECT
    rbw.fighter_id,
    AVG(rbw.opp_tier_value::float) AS streak_opp_avg_tier
  FROM recent_bouts_window rbw
  JOIN fighter f ON f.id = rbw.fighter_id
  WHERE COALESCE(f.current_streak, 0) >= 1
    AND rbw.recency_rank <= COALESCE(f.current_streak, 0)
  GROUP BY rbw.fighter_id
),
...
components AS (
  SELECT
    ...
    COALESCE(soq.streak_opp_avg_tier, 0)::float AS streak_opp_avg_tier,

    -- Wave 6E.4: smooth linear activity fade replaces step-bucket logic.
    -- Active rostered: 100 at ≤6mo → linear to 50 at ≥18mo. Non-active:
    -- 80 at <9mo → linear to 0 at ≥24mo. has_upcoming_bout overrides
    -- both paths to 100.
    CASE
      WHEN f.has_upcoming_bout THEN 100.0
      WHEN f.roster_status = 'active' AND msl.m IS NULL THEN 100.0
      WHEN f.roster_status = 'active' AND msl.m <= 6 THEN 100.0
      WHEN f.roster_status = 'active' AND msl.m >= 18 THEN 50.0
      WHEN f.roster_status = 'active' THEN
        GREATEST(50.0, 100.0 - (msl.m - 6.0) * (50.0 / 12.0))
  ...
  LEFT JOIN streak_opp_quality soq ON soq.fighter_id = f.id
...
raw_scores AS (
  SELECT
    *,
    -- Wave 6E.4: quality-adjusted streak bonus. Base bonus =
    -- LEAST(streak,5)*4 (max 20). Multiplied by streak_opp_avg_tier/15
    -- capped at 1.0. Gate (current_streak ≥ 3 AND quality_wins ≥ 20)
    -- preserved from prior.
    CASE
      WHEN current_streak >= 3 AND quality_wins >= 20
      THEN LEAST(current_streak, 5)::float * 4.0
           * LEAST(1.0, GREATEST(0.0, streak_opp_avg_tier / 15.0))
      ELSE 0::float
