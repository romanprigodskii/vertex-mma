-- Wave 6E.4.4: tier-discounted recent_loss_penalty + activity 12→24 fade.
--
-- (1) recent_loss_penalty now scales each loss by opponent quality.
--     Component breakdown of Della Maddalena (cur=31, raw=24) revealed
--     the bug: he lost back-to-back to Islam Makhachev (apex P4P#1,
--     opp_tier=25) and Carlos Prates (top-5, opp_tier=22) and was
--     penalized as if losing twice to ranked-tier (rp=86 → -17.2 raw
--     → ~-23 final). By symmetry with quality_wins (which rewards
--     beating elite more), losses should be discounted by opp quality.
--
--     New per-loss severity:
--       severity = clamp(0.3, 1.0, 1.0 - opp_tier_value / 30.0)
--     opp_tier  0 (prospect) → 1.00 (full penalty)
--     opp_tier  3 (ranked)   → 0.90
--     opp_tier  8 (solid)    → 0.73
--     opp_tier 14 (top10)    → 0.53
--     opp_tier 22 (top5)     → 0.30 (floor — still counts)
--     opp_tier 25 (apex)     → 0.30 (floor)
--
--     The 0.30 floor prevents "lose to anyone elite = free pass"
--     while still meaningfully discounting losses to clear standouts.
--
--     Layered structure (×25/×12/×6 over last_3/last_5/24mo) preserved.
--     Reuses bout_opponent_tier from Wave 6E.4 Part A.
--
-- (2) activity smooth fade extended from 6→18 mo to 12→24 mo. Half-
--     year layoffs no longer penalize (the championship rotation is
--     ~9-12 months, not 6). Full year layoff = fade begins. Two years
--     = minimum 50. Non-rostered logic unchanged (still 9 / 24 mo).
--
-- Locked-in (unchanged): all-time formula (uses total_loss_penalty,
-- not recent_loss_penalty), other component weights, multiplier curve,
-- skid penalty (3+ losses in last 3 still triggers -25), ufc_bouts
-- gates, championship_pedigree (with 6E.2 sticky rule), quality_wins
-- multipliers, streak quality scaling (from 6E.4 Part B),
-- era_dominance current/all-time, performance_diff_current,
-- finishing_dominance hybrid (from 6E.4.3 with ×35/×90 multipliers).
