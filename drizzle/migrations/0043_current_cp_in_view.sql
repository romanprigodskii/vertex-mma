-- Wave 12: current_cp column + view consumes it for current_score.
--
-- New fighter.current_cp column (smallint, default 0) replaces
-- championship_pedigree in the current_score formula only. all_time
-- formula keeps using championship_pedigree (eternal legacy). UI tier
-- classification (vertex-tier.ts) continues to read championship_pedigree.
--
-- current_cp is populated by scripts/compute_current_cp.ts (run after
-- compute_championship_pedigree.ts). It applies time-decay + recent-opp
-- bonus to former-champion buckets (80/70); other buckets (100/90/40/0)
-- pass through unchanged. See compute_current_cp.ts for the V4c curves
-- and bonus thresholds.
--
-- This migration:
--   (1) ALTER TABLE fighter ADD COLUMN current_cp SMALLINT NOT NULL DEFAULT 0
--   (2) DROP/CREATE the fighter_vertex_score view so the components CTE
--       exposes both championship_pedigree AND current_cp, and raw_current
--       uses current_cp * 0.12 in place of championship_pedigree * 0.12.
--   The all_time_score expression at the bottom of the SELECT is unchanged
--   (still references championship_pedigree * 0.22).
--
-- All other CTEs, weights, multiplier curve, gates, skid penalty,
-- recent_form_score (Wave 9), recent_loss_penalty (Wave 6E.4.4),
-- activity fade (12→24 from 6E.4.4), finishing_dominance hybrid (6E.4.3),
-- and era_dominance_current — unchanged from migration 0042.

ALTER TABLE fighter ADD COLUMN IF NOT EXISTS current_cp SMALLINT NOT NULL DEFAULT 0;
...
components AS (
  SELECT
    ...
    -- Wave 12: time-decayed + opp-bonus variant used in current_score.
    -- Populated by scripts/compute_current_cp.ts; falls back to base CP
    -- if compute hasn't run yet (defensive: COALESCE against the column
    -- default 0 is fine because 0 → no champ credit, matches base 0
    -- bucket behavior).
    ...
    *,
    CASE
      WHEN is_active AND ufc_bouts >= 5 THEN
        GREATEST(0,
            quality_wins * 0.22
          -- Wave 12: current_cp (time-decayed + opp-bonus) replaces
          -- championship_pedigree in the current_score raw computation.
          -- For active champions / sticky-100 / lost-challenger / never
          -- buckets the two values coincide. Weight unchanged at 0.12.
          + current_cp * 0.12
