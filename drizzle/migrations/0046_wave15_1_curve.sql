-- Wave 15.1: recalibrate the soft-multiplier curve for the post-Wave-15
-- raw range.
--
-- Wave 15's decay + reduced positive-weight sum (0.88 vs ~1.00 pre-W15)
-- compressed raw_current from the old ~50..150 range down to ~25..65.
-- The Wave 15 curve was calibrated for the old range (zones <40 / 40-65
-- / 65-85 / 85+); under the new compression, every champion-tier fighter
-- lands in the 40-65 zone and gets the same anchored-slope treatment,
-- hiding the formula's real intent and producing visible 15-20 point
-- drops on dominant champions.
--
-- Wave 15.1 is pure calibration: the only change is the multiplied CTE's
-- CASE expression. The new curve uses flat multipliers per zone aligned
-- to the new raw range (<25 / 25-45 / 45-60 / 60+):
--   raw <  25 → × 1.0    (clear floor — weak fighters stay where they are)
--   raw 25-45 → × 1.30   (sub-elite contender band lifts toward 30-60)
--   raw 45-60 → × 1.45   (elite contender band lifts toward 65-87)
--   raw ≥ 60  → × 1.50   (top-of-class lifts toward 90+)
-- Final LEAST(100, …) clamp preserves the 0..100 ceiling.
--
-- Skid penalty (-25 at losses_last_3 ≥ 3), all raw_current components
-- and weights, the all-time branch, and the ufc_bouts ≥ 5 gate are
-- unchanged from Wave 15 (migration 0045). The view column list is
-- identical, so consumers (materialize_vertex_score, diagnostics) need
-- no changes.
...
multiplied AS (
  SELECT
    *,
    -- Wave 15.1: flat multiplier per zone, calibrated for the
    -- post-Wave-15 raw range (~25..65 across active rostered fighters).
    --   raw <  25 → × 1.0
    --   raw 25-45 → × 1.30
    --   raw 45-60 → × 1.45
    --   raw ≥ 60  → × 1.50
