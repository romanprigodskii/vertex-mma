-- Wave 22: performance_diff_current rebuilt as max-of-three with a
-- PROPORTIONAL balance bonus. Specialists (Pereira/Khabib) keep full
-- credit via their strongest dimension; universalists get a bonus
-- scaled by their max — mediocre balanced fighters don't get a free
-- floor lift.
--
-- Old formula (Wave 15.1): weighted sum
--     0.45·(striking_dom + Δ·0.5)
--   + 0.35·(control_dom  + Δ·0.3)
--   + 0.20·(td_dom)
--
-- New formula:
--     max   = GREATEST(striking_d, control_d, td_d)
--     min   = LEAST  (striking_d, control_d, td_d)
--     ratio = min / GREATEST(max, 1)        — 0..1 universalist fraction
--     performance_diff_current = LEAST(100, max * (1 + 0.25 * ratio))
--
-- Behaviour at illustrative points:
--   Pereira (100, 0, 0)   max=100, ratio=0   → 100
--   GSP-like (75, 75, 75) max=75,  ratio=1   → 93.75
--   Mid 50/50/50          max=50,  ratio=1   → 62.5   (was 75 in flat-bonus)
--   80/50/40              max=80,  ratio=0.5 → 90
--   70/0/0 weak one-dim   max=70,  ratio=0   → 70
--
-- The first iteration of this wave used a flat `+ ratio*25` bonus which
-- lifted neutral baselines from ~50 to 75 across the roster, drifting
-- almost every active fighter +5 to +15. Proportional scaling keeps the
-- specialist/universalist intent without the floor shift.
--
-- The three dimensions are computed in a new CTE
-- `recent_performance_dims` between recent_performance and components.
-- striking_d and control_d carry the same Δ corrections as the old sum
-- (0.5× and 0.3×) so opp-quality stays baked in; td_d uses the raw
-- recent_td_dominance.
--
-- Wave 21 changes (opp-tier-weighted finishing_dominance_decayed +
-- layered activity in components) carried over unchanged from migration
-- 0049. Everything else (quality_wins_decayed, era_dominance_current,
-- recent_form_score, recent_loss_penalty, curve thresholds, skid
-- penalty, all-time formula) is byte-identical to 0049.
