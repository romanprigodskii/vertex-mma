-- Wave 23: smooth multiplier curve. Replaces the Wave 15.1 step
-- function in the `multiplied` CTE with a continuous piecewise-linear
-- curve that passes through the same anchor points. Eliminates the
-- visible distribution gaps at raw_current ≈ 25, 45, 60 caused by the
-- step boundaries.
--
-- Old curve (step, from 0046 / kept through 0050):
--   raw <25      → raw × 1.00
--   raw 25..45   → raw × 1.30
--   raw 45..60   → raw × 1.45
--   raw ≥60      → raw × 1.50
-- Discontinuities at 25 (25 → 32.5), 45 (45 → 65.25 vs 58.5),
-- 60 (60 → 90 vs 87) — observed as 6–7 point empty bands in the
-- active-roster distribution after Wave 22 lifted raw_current.
--
-- New curve (continuous piecewise-linear through anchor points):
--   (raw, multiplied) = (0, 0), (25, 25), (45, 65.25), (60, 90),
--                       (67, 100), cap at 100 above 67.
-- Continuity at every join:
--   raw=25 → 25 (both sides)
--   raw=45 → 65.25 (both sides)
--   raw=60 → 90 (both sides)
--   raw=67 → 100 (both sides)
-- Slope per segment:
--   0..25   : 1.0
--   25..45  : 2.0125   ((65.25 - 25) / 20)
--   45..60  : 1.65     ((90 - 65.25) / 15)
--   60..67  : 10/7     ((100 - 90) / 7)
--   ≥67     : 0 (cap)
--
-- Effect (approximate, computed against current raw_current values):
--   Pereira (raw 70.0)   100 → 100   unchanged
--   Merab   (raw 62.6)    94 → 94    unchanged
--   Islam   (raw 61.5)    92 → 92    unchanged
--   Topuria (raw 58.8)    85 → 88    fills 86–91 gap
--   Aspinall(raw 53.9)    78 → 80
--   Holloway(raw 47.6)    69 → 70
--   raw 30                39 → 35    -4 (price of low-edge continuity)
--   raw 25                33 → 25    -8 (price of low-edge continuity)
--   raw <25               unchanged
--
-- Nothing else changes. raw_current formula (Wave 22 max-of-three +
-- balance bonus), all-time formula, skid penalty, weights, recency
-- handling, opp-tier weighting — all byte-identical to 0050.
