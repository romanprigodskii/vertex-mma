-- Wave 24: recent_form_score 1.5× stretch. Per-bout impact on the
-- formula's largest-weighted component (RFS, weight 0.18 in raw_current)
-- doubled from /5 to ×0.30, so SUM(contribution) lands fully on
-- 50 + SUM × 0.30 instead of being divided by sample size.
--
-- Old formula (kept through 0051):
--   recent_form_score = clamp(50 + SUM(contribution) / 5)
--   per-bout impact: contribution × 0.20
-- New formula:
--   recent_form_score = clamp(50 + SUM(contribution) × 0.30)
--   per-bout impact: contribution × 0.30  (1.5× the old slope)
--
-- Contribution itself (win = +opp_tier_value; loss = -opp_tier_value
-- × opp-tier-discount; no_contest = 0) is unchanged. Sample window
-- (recency_rank ≤ 5) is unchanged. Weight in raw_current (× 0.18) is
-- unchanged. Only the inside of the SUM-to-RFS mapping moves.
--
-- Why: elite recent form was compressed in the 57–72 band — Pereira's
-- 4-1 vs apex read almost the same as Joshua Van's 4-0-1 vs mid-tier.
-- Stretch lifts the elite ceiling to 75–85 while leaving mid-roster
-- close to baseline 50.
--
-- Effect on RFS (last 5 bouts SUM → RFS):
--   Pereira   SUM 90   65 → 77
--   Islam     SUM 110  72 → 83
--   Topuria   SUM 95   69 → 79
--   Merab     SUM 85   67 → 76
--   Joshua V  SUM 70   64 → 71
--   Valentina SUM 60   62 → 68
--   Aspinall  SUM 50   60 → 65
--   Holloway  SUM 35   57 → 61
--   mid 3-2   SUM 15   53 → 55
--   5L vs apex SUM -37 42 → 39
--
-- Effect on vertex_score (RFS Δ flows through curve via × 0.18 weight):
--   Pereira    100 → 100  (cap)
--   Islam      92 → 95
--   Merab      94 → 96
--   Joshua Van 92 → 94
...
--   Weili      74 → 77
--
-- Multiplier curve (Wave 23), raw_current formula (Wave 22), all-time
-- formula, skid penalty, weights, recency handling — all byte-identical
-- to 0051.
