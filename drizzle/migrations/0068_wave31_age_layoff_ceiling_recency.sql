-- Wave 31: five layered changes to the current_score branch of
-- fighter_vertex_score. all-time formula UNCHANGED (carryover from Wave 15+).
--
-- Motivation snapshot (from Wave 30 calibration audit):
--   Pereira  → vertex_score 100  (raw ~70, hits Wave 23 ceiling at raw 67)
--   Merab    → vertex_score 96   (raw ~62.6, still > Islam despite a recent
--                                 destructive title-fight loss)
--   Islam    → vertex_score 92   (raw ~61.5)
--
-- Two structural failures explain the above:
--   (a) Wave 23 curve maps raw 67..88 to a single point (100). Positive
--       weights sum to 0.88, so theoretical max raw_current is 88, and the
--       Wave 23 anchor (67, 100) compresses ~21 points of headroom into the
--       ceiling. Top fighters become indistinguishable.
--   (b) A single recent loss does NOT trigger Wave 30 skid penalty (needs
--       2+ in last 3). recent_loss_penalty is heavily severity-discounted
--       for apex-tier losses (severity floor 0.3 → -2.6 in raw_current per
--       loss to a champion). recent_form_score sums contributions equally
--       across the 5-bout window with no recency weight. Net: a fresh
--       loss costs ~3 raw points even when devastating.
--
-- Five changes in this migration:
--
--   1. AGE CURVE — multiplicative adjustment on raw_current. fighter.dob is
--      already on the table, never used in scoring before. Adds prime/decline
--      shape:
--        < 26      → 0%     (still developing — neutral)
--        26..30    → +5%    (prime)
--        31..32    → +2%
--        33..34    → 0%
--        35..36    → -4%
--        37..38    → -8%
--        ≥ 39      → -12%
--      Applied to current only. all-time formula is legacy and shouldn't
--      decay by age. dob NULL → 0% (no signal, treat as neutral).
--
--   2. LAYOFF PENALTY — additive penalty on raw_current using
--      months_since_last (CTE already exists, just unused). Different signal
--      from activity (volume in window): this is "how long since the last
--      fight", which captures rust irrespective of past frequency.
--        m ≤ 12     → 0
--        m ≤ 42     → (m - 12) × 0.5     (0..15)
--        m > 42     → 15                  (cap)
--      Stacks with activity. A fighter with 0 fights in last 24mo gets
--      activity ≈ 0 AND layoff ≈ 6+, which is correct: lack of volume AND
--      lack of recency are independent signals.
--
--   3. CURVE RE-ANCHOR — replaces Wave 23 (0,0)/(25,25)/(45,65.25)/(60,90)/
--      (67,100) with a curve that uses the full positive headroom:
--        (0,0), (25,25), (45,60), (60,80), (75,93), (88,100)
--      Slopes 1.0 / 1.75 / 1.333 / 0.867 / 0.538 / cap.
--      Effect on Wave 23 calibration points (approximate, before age/layoff):
--        Pereira  raw 70.0  → 88.7  (was 100)
--        Merab    raw 62.6  → 82.3  (was 94)
--        Islam    raw 61.5  → 81.3  (was 92)
--        Topuria  raw 58.8  → 78.4  (was 88)
--        Aspinall raw 53.9  → 71.9  (was 80)
--        Holloway raw 47.6  → 63.5  (was 70)
--      Top size 75-90 instead of clustering at 92-100. 100 reserved for
--      raw ≥ 88, which requires near-perfect positive components with
--      zero penalties — practically unreachable today.
--
--   4. RECENCY-WEIGHTED recent_form_score — inside the 5-bout window,
--      weight contributions by rn_desc:
--        rn=1 → 2.00,  rn=2 → 1.50,  rn=3 → 1.00,  rn=4 → 0.75,  rn=5 → 0.50
--      Sum of weights 5.75. New formula:
--        50 + (SUM(contribution × weight) / SUM(weight)) × 1.50
--      All-wins case stays identical (avg = +opp_tier, ×1.5 matches old
--      ×0.3 scale). Mixed case: a fresh loss bites harder than an old one.
--      Merab scenario (4 prior apex wins + 1 fresh apex loss):
--        old form = 78,  new form = 71  → -7 form pts → -1.26 in raw.
--
--   5. FRESH-LOSS FLAT PENALTY — if the most recent completed bout
--      (rn_desc=1) was a loss, apply a flat -5 to multiplied_current as a
--      final step. Stacks with skid penalty branches (skid catches first
--      via the existing CASE ordering). Captures the "you just lost,
--      doesn't matter how good they were" signal that severity-discounting
--      hides today.
--
-- Net effect on the calibration trio (approximate, full pipeline):
--   Pereira:  raw 70 → age × 0.92 = 64.4 → curve 83.8 → no fresh loss = 84
--             (was 100, drop ~16)
--   Islam:    raw 61.5 → age × 1.00 = 61.5 → curve 81.3 → no fresh loss = 81
--             (was 92, drop ~11)
--   Merab:    raw 62.6 → recency form -1.26 = 61.3 → age × 0.96 = 58.85 →
--             curve 78.5 → fresh loss = 73.5 → 74
--             (was 96, drop ~22; now Islam > Merab as expected)
--
-- Re-materialization required after applying:
--   pnpm tsx scripts/materialize_vertex_score.ts
--   pnpm tsx scripts/materialize_divisional_score.ts
--
-- Wave 31 changes ONLY:
--   - Adds CTE: most_recent_bout, fighter_age_calc
--   - Modifies CTE: recent_form (recency weights)
--   - Modifies: components (exposes age + layoff + most_recent_is_loss)
--   - Modifies: raw_scores (layoff term + age multiplier)
--   - Modifies: multiplied (re-anchored curve)
--   - Modifies: final SELECT vertex_score CASE (fresh-loss branch)
--
-- All-time formula and every other CTE — byte-identical to Wave 30 (0054).
