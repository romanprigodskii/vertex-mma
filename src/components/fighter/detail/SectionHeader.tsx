...
export function SectionHeader({ label, explainer }: SectionHeaderProps) {
  return (
    <header className="mb-5 flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
      <h2 className="font-sans text-[13px] font-medium uppercase tracking-widest text-foreground-muted">
        {label}
      </h2>
      {explainer ? (
        <p className="max-w-prose font-sans text-sm text-foreground-subtle">
          {explainer}
        </p>
      ) : null}
    </header>
