...
export function BetForm({ market, userBalance }: Props) {
  ...
  // Optimistic balance: until router.refresh() brings the new server prop, a
  // just-placed bet's stake is reflected here so the form validates against the
  // REDUCED balance (otherwise the reset stake could exceed the real balance
  // and only fail server-side). Reset when a fresh server balance arrives —
  // the React-sanctioned "adjust state during render when a prop changes"
  // pattern (no effect, so no set-state-in-effect concern).
  const prevBalance = React.useRef(userBalance);
  const [spent, setSpent] = React.useState(0);
  if (prevBalance.current !== userBalance) {
    prevBalance.current = userBalance;
    if (spent !== 0) setSpent(0);
  }
  const available = Math.max(0, userBalance - spent);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // Reflect the spend optimistically and re-default the stake to something
    // still affordable (never a fixed 100 that may now exceed the balance).
    const spentNow = res.coinsSpent ?? parsedCoins;
    setSpent((s) => s + spentNow);
    setCoins(String(defaultStake(Math.max(0, available - spentNow))));
    setPreview(null);
    router.refresh();
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <button
          type="submit"
          disabled={pending || parsedCoins < 1 || parsedCoins > available}
